...
TEST_F(ConfigTests, isFrontendFunctionNullptrIsNotAFrontendFunction)
{
	parseInput(R"(
		declare void @frontendFunction()
	)");
	auto config = Config::empty(module.get());
	config.getConfig().parameters.frontendFunctions.insert("frontendFunction");

	EXPECT_FALSE(config.isFrontendFunction(nullptr));
}

TEST_F(ConfigTests, isFrontendFunctionCorrectlyDecidesWhichFunctionIsAFrontendFunction)
{
	parseInput(R"(
		declare void @frontendFunction()
		define void @fnc() {
	...
	Function* frontendFnc = getFunctionByName("frontendFunction");
	Function* fnc = getFunctionByName("fnc");
	auto config = Config::empty(module.get());
	config.getConfig().parameters.frontendFunctions.insert("frontendFunction");

	ASSERT_NE(nullptr, frontendFnc);
	ASSERT_NE(nullptr, fnc);
	EXPECT_TRUE(config.isFrontendFunction(frontendFnc));
	EXPECT_FALSE(config.isFrontendFunction(fnc));
}

//
// isFrontendFunctionCall()
//

TEST_F(ConfigTests, isFrontendFunctionCallNullptrIsNotAFrontendFunctionCall)
{
	parseInput(R"(
		declare void @frontendFunction()
	)");
	...
	EXPECT_FALSE(config.isFrontendFunctionCall(nullptr));
}

TEST_F(ConfigTests, isFrontendFunctionCallCorrectlyDecidesWhichFunctionCallIsAFrontendFunctionCall)
{
	parseInput(R"(
		declare i32 @frontendFunction()
		define i32 @fnc() {
			%a = call i32 @frontendFunction()
			%b = call i32 @fnc()
			ret i32 %a
		}
	)");
	Instruction* frontendFncCall = getInstructionByName("a");
	Instruction* fncCall = getInstructionByName("b");
	auto config = Config::empty(module.get());
	config.getConfig().parameters.frontendFunctions.insert("frontendFunction");

	ASSERT_NE(nullptr, frontendFncCall);
	ASSERT_NE(nullptr, fncCall);
	EXPECT_TRUE(config.isFrontendFunctionCall(frontendFncCall));
	EXPECT_FALSE(config.isFrontendFunctionCall(fncCall));
}
...
//
// isLlvmToAsmGlobalVariable()
//

TEST_F(ConfigTests, isLlvmToAsmGlobalVariableReturnsTrueIfSet)
{
	parseInput(R"(
		@llvm2asm = global i64 0
	)");
	auto* gv = getGlobalByName("llvm2asm");
	auto config = Config::empty(module.get());
	config.setLlvmToAsmGlobalVariable(gv);

	EXPECT_TRUE(config.isLlvmToAsmGlobalVariable(gv));
}

TEST_F(ConfigTests, isLlvmToAsmGlobalVariableReturnsFalseIfNotSet)
{
	parseInput(R"(
		@dummy = global i64 0
		@llvm2asm = global i64 0
	)");
	auto* dummy = getGlobalByName("dummy");
	auto* gv = getGlobalByName("llvm2asm");
	auto config = Config::empty(module.get());
	config.setLlvmToAsmGlobalVariable(dummy);

	EXPECT_FALSE(config.isLlvmToAsmGlobalVariable(gv));
}
...
TEST_F(ConfigTests, isLlvmToAsmInstructionReturnsTrueIfItSetsSpecialGlobal)
{
	parseInput(R"(
		@llvm2asm = global i64 0
		define void @fnc() {
			store i64 123, i64* @llvm2asm
			ret void
		}
	)");
	auto* gv = getGlobalByName("llvm2asm");
	...
	EXPECT_TRUE(config.isLlvmToAsmInstruction(s));
}

TEST_F(ConfigTests, isLlvmToAsmInstructionReturnsFalseIfItDoesNotSetSpecialGlobal)
{
	parseInput(R"(
		@dummy = global i64 0
		@llvm2asm = global i64 0
		define void @fnc() {
			store i64 123, i64* @dummy
			ret void
		}
	)");
	auto* gv = getGlobalByName("llvm2asm");
	...
	EXPECT_FALSE(config.isLlvmToAsmInstruction(s));
}
...
TEST_F(ConfigTests, getLlvmToAsmGlobalVariableReturnsVariableIfSet)
{
	parseInput(R"(
		@llvm2asm = global i64 0
	)");
	auto* gv1 = getGlobalByName("llvm2asm");
	auto config = Config::empty(module.get());
	config.setLlvmToAsmGlobalVariable(gv1);
	auto* gv2 = config.getLlvmToAsmGlobalVariable();

	EXPECT_NE(nullptr, gv2);
	EXPECT_EQ(gv1, gv2);
}

TEST_F(ConfigTests, getLlvmToAsmGlobalVariableReturnsNullptrIfNotSet)
{
	parseInput(R"(
		@dummy = global i64 0
	)");
	auto config = Config::empty(module.get());
	auto* gv = config.getLlvmToAsmGlobalVariable();
