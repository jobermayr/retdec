...
BIN2LLVMIR_PARAMS="-provider-init -decoder -asm-inst-opt -inst-opt -main-detection -register -cond-branch-opt -syscalls -idioms-libgcc -stack -constants -param-return -local-vars -type-conversions -simple-types -generate-dsm -remove-asm-instrs -select-fncs -unreachable-funcs -type-conversions -class-hierarchy -value-protect $BIN2LLVMIR_LLVM_PASSES_ONLY $BIN2LLVMIR_LLVM_PASSES_ONLY -simple-types -stack-ptr-op-remove -type-conversions -idioms -global-to-local -dead-global-assign -instcombine -phi2seq -value-protect $BIN2LLVMIR_PARAMS_DISABLES"
