...
BIN2LLVMIR_PARAMS="-provider-init -decoder -inst-opt -main-detection -register -stack -cond-branch-opt -syscalls -idioms-libgcc -constants -param-return -local-vars -type-conversions -simple-types -generate-dsm -remove-asm-instrs -select-fncs -unreachable-funcs -type-conversions -class-hierarchy -stack-protect $BIN2LLVMIR_LLVM_PASSES_ONLY $BIN2LLVMIR_LLVM_PASSES_ONLY -simple-types -stack-ptr-op-remove -type-conversions -idioms -global-to-local -dead-global-assign -instcombine -phi2seq -stack-protect $BIN2LLVMIR_PARAMS_DISABLES"
