...
BIN2LLVMIR_PARAMS="-provider-init -decoder -idioms-libgcc -inst-opt -main-detection -register -cond-branch-opt -syscalls -stack -constants -param-return -local-vars -inst-opt -simple-types -generate-dsm -remove-asm-instrs -select-fncs -unreachable-funcs -inst-opt -class-hierarchy -value-protect $BIN2LLVMIR_LLVM_PASSES_ONLY $BIN2LLVMIR_LLVM_PASSES_ONLY -simple-types -stack-ptr-op-remove -inst-opt -idioms -global-to-local -dead-global-assign -instcombine -phi2seq -value-protect $BIN2LLVMIR_PARAMS_DISABLES"
